...
public class OrderService(OrderContext context, ILogger<OrderService> logger, HttpClient httpClient)
{
    private const string PendingTransactionId = "pending";
    private readonly OrderContext _context = context;

    public async Task<OrderCreateResponse> CreateAsync(ClaimsPrincipal user, Order orderIn, string authorization,
        CancellationToken cancellationToken)
    {
        ...
        var savedOrder = await SaveOrder(order, cancellationToken);
    ...
    private async Task<Order> SaveOrder(Order order, CancellationToken cancellationToken)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await response.Content.ReadFromJsonAsync<Payment>(cancellationToken) ?? new Payment();
    }
